...
export async function POST(request: NextRequest) {
  ...
  // When a date range is selected (week/month), bound activities to that window.
  // When "all dates" is selected (afterDate is null), include future activities too —
  // cells from activities that haven't happened yet will be left uncolored below.
  const tomorrow = new Date();
  tomorrow.setUTCDate(tomorrow.getUTCDate() + 1);
  const tomorrowStart = new Date(tomorrow.toISOString().split("T")[0] + "T00:00:00Z");
  const dateFilter = afterDate ? { gte: afterDate, lt: tomorrowStart } : {};
  const now = new Date();
