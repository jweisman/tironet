...
setup("seed database and authenticate users", async ({ browser }) => {
  ...
  // 3. Log in each user role and save storageState
  await loginAndSaveState(browser, EMAILS.admin, "e2e/.auth/admin.json");
  await loginAndSaveState(browser, EMAILS.platoonCmd, "e2e/.auth/platoon-cmd.json");
  await loginAndSaveState(browser, EMAILS.squadCmd, "e2e/.auth/squad-cmd.json");
});
