...
- **Calendar** — Monthly calendar view showing activities, leave requests, medical appointments, sick days, commander events, and home visits across the user's scope; color-coded by platoon or event type with filtering; mobile-optimized with iOS Calendar-style dots and day detail list; PDF export (landscape A4, one month per page); available to all roles including squad commanders
- **Reports** — Activity summary (PDF with pie charts and grade tables by unit), all-scores matrix (Google Sheets, soldiers × scored activities with per-cell failure coloring), physical training tracker (Google Sheets, IDF מדא"גיות format), approved request summary (PDF grouped by unit), and personal file (תיק אישי) per soldier (PDF); all scoped by role and filterable by type
...
- **Soldier profiles** — Photo upload with in-browser cropping and compression; emergency contact (name, relationship, phone); incidents (commendations and infractions); home visit tracking; bulk import from Excel
...
## Infra
Several infrastructure utilities are located in the `infra` directory.

## Credits

### Definition and Implementation
This app was defined and implemented by Aron Weisman.
