...
- **Reports** — Activity summary (PDF with pie charts and grade tables by unit), all-activity matrix (Google Sheets), physical training tracker (Google Sheets, IDF מדא"גיות format), approved request summary (PDF grouped by unit), and personal file (תיק אישי) per soldier (PDF); all scoped by role and filterable by type
